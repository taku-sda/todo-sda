	...
	<div class="container">
		...
		<div class="bg-light p-2 mb-5">
			...
			<form action="/LoggedIn/AddItem" method="post">
						...
						<label> <select name="year">
								<c:forEach var="i" begin="2020" end="2099" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == now.getYear()}">selected</c:if>>
								...
								<c:forEach var="i" begin="1" end="12" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == now.getMonthValue()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>月
								...
								<c:forEach var="i" begin="1" end="31" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == now.getDayOfMonth()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>日
				...
				<div class="form-check form-check-inline">
					<input type="radio" class="form-check-input" name="importance"
						id="importance2" value="2" checked>
