			...
			<form action="/LoggedIn/UpdateItem" method="post">
				...
				<div class="from-group w-75">
					...
					<input type="text"class="form-control" name="title" id="title"
					 	maxlength="20" value="${detailItem.getTitle()}" required>
				</div>
						...
						<label> <select name="year">
								<c:forEach var="i" begin="2020" end="2099" step="1">
									<option value=<c:out value="${i}"> </c:out>
										<c:if test="${i == detailItem.getYear()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>年
						</label> <label> <select name="month">
								<c:forEach var="i" begin="1" end="12" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == detailItem.getMonth()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>月
								...
								<c:forEach var="i" begin="1" end="31" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == detailItem.getDay()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>日
								...
								<c:forEach var="i" begin="0" end="23" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == detailItem.getHour()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>時
								...
								<c:forEach var="i" begin="0" end="59" step="1">
									<option value=<c:out value="${i}"></c:out>
										<c:if test="${i == detailItem.getMinute()}">selected</c:if>>
										<c:out value="${i}"></c:out></option>
								</c:forEach>
						</select>分
						</label>
				...
				<div class="form-check form-check-inline">
					<input type="radio" class="form-check-input" name="importance"
				...
				<div class="form-check form-check-inline">
					...
					<label class="form-check-label" for="importance1"> 小 </label>
				</div>
